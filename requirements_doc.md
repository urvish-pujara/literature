...
- **Hand Privacy:** Client payloads must _only_ contain that specific player's cards. The server must never emit the full game state to any client.
...
4. **State Transfer (Joker):** If Player B holds one or both Jokers, the server deducts exactly _one_ Joker from Player B's hand and transfers it to Player A.
